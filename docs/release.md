...
1. **Own the `@arcsig-labs` scope on npm.** A scope belongs to the user or organisation of the same name. The organisation `arcsig-labs` was created on 2026-09-21 (free plan, public packages); `@arcflow` itself belongs to an unrelated user, which is why the packages are not published under the project's own name. No package is published under `@arcsig-labs` yet.
2. **Make a publish token.** Access Tokens → Generate → *Granular access token*: read and write, restricted to packages and scopes `@arcsig-labs`, and *Bypass two-factor authentication* on, because CI publishes without a person present. Copy it once.
...
2. **Pack and try the tarballs** the way a user would receive them. `pnpm pack` rewrites `workspace:*` and applies `publishConfig`, and copies the root `LICENSE` into each package.
   ...
   Installing all tarballs in one `npm install` lets npm satisfy the `@arcsig-labs/*` dependencies from each other instead of the registry, which matters before the first publish.
...
3. **Say what the version is.** In one commit:
   ...
   - If the version number changes, change it in every `packages/*/package.json` and in `@arcsig-labs/server`'s peer ranges.
...
5. **Confirm from the outside.**
   ...
   ```sh
   npm view @arcsig-labs/core version
   npx -y @arcsig-labs/mcp --help
   npx -y @arcsig-labs/server --help
   ```
